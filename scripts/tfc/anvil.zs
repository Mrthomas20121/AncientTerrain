...
for key in anvilRecipes {
  if(key == "bismuth" || key == "rose_gold") {
    ...
    Anvil.addRecipe('tinker_propick_' + key, itemUtils.getItem("tfc:metal/ingot/" + key) , <tinkerfirmacraft:propick_head>.withTag({Material: key}), anvilRecipes[key], "tools",  "PUNCH_LAST","DRAW_NOT_LAST","BEND_NOT_LAST");
    ...
    recipes.addShapeless('tconstruct/tinker/propick/' + key, <tinkerfirmacraft:propick_head>.withTag({Material: key}), [itemUtils.getItem("tfc:metal/propick_head/" + key)]);
    ...
    recipes.addShapeless('tfc/metal/propick_head/' + key, itemUtils.getItem("tfc:metal/propick_head/" + key), [<tinkerfirmacraft:propick_head>.withTag({Material: key})]);
  }
}